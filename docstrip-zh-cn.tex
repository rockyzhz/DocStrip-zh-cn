...
这些宏指定了如果要生成的文件已经存在时应该发生什么。如果 |\askforoverwritetrue| 处于激活状态（默认情况下），会询问用户
是否应该覆盖文件。但是如果已经使用了 |\askforoverwritefalse|，则现有文件将会被静默地覆盖。这些开关是局部的，可以在文件
的任何地方发出，甚至在 |\generate| 子句内部（但在 |\file| 之间）。
...
\begin{itemize}
    ...
    \item[\texttt{<}] 当在输入中发现一个“守护行”并且它开始一个可选包含的代码块时，在终端上通过显示 \texttt{<}-字符来表示，
          并伴随着守护的布尔表达式。
...
代码的支持实现的。在文档文件中，可选代码是通过一个“守护”来标记的。

守护是一个布尔表达式，它被包含在 |<| 和 |>| 中。它还{\em 必须\/}紧跟在行首的 |%| 后面。例如：
\begin{verbatim}
   ...
   %<bool>\TeX code
   ...
...
为了区分这两种可选代码，引入了“守护修饰符”。它是紧跟在守护的 |<| 后面的一个字符。它可以是 |*| 表示代码块的开始，
也可以是 |/| 表示代码块的结束\footnote{为了与早期版本的 \ds{} 兼容，也支持 \texttt{+} 和 \texttt{-} 作为“守护
    修饰符”。但是，与先前行为相比，与一个 \texttt{+} 修饰的守护对应的行不会包含在求值为假的守护块内，这里存在不兼容。}。
代码块的开始和结束守护必须单独占据一行。

当一个代码块{\em 不\/}被包含时，该块内出现的任何守护都{\em 不\/}会被评估。
...
要使用 \ds 提取代码，原始的“守护”机制被扩展，引入了语法 \texttt{\%<@@=\meta{module}>}。当提取代码时，
